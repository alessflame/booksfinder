...
window.onscroll=()=>{
animazione(info[0]); animazione(info[1]); 
...
catch(err){sezioneLibri.innerHTML = "Qualcosa &egrave andato storto";}
};
...
async function readBook(e){
   ...
   try{
    ...
    addStars.onclick=()=>{starsBook(response.title, response.image);  
    }}
    catch(err){
...
sezioneLibri.addEventListener("click",(e)=> {readBook(e);});
